...
public class Benchmark01 : MonoBehaviour
{
	...
	private IEnumerator Start()
	{
		...
		else if (BenchmarkType == 1)
		{
			...
			{
				_003F val = m_textMesh;
				Object obj = Resources.Load("Fonts/ARIAL", typeof(Font));
				((TextMesh)val).font = (Font)(object)((obj is Font) ? obj : null);
				((Component)m_textMesh).GetComponent<Renderer>().sharedMaterial = m_textMesh.font.material;
			}
		...
		for (int i = 0; i <= 1000000; i++)
		{
			if (BenchmarkType == 0)
			{
				...
				if (i % 1000 == 999)
				{
					...
					{
						Material val2 = (((TMP_Text)m_textMeshPro).fontSharedMaterial = m_material01);
						fontSharedMaterial = val2;
					}
					else
					{
						Material val2 = (((TMP_Text)m_textMeshPro).fontSharedMaterial = m_material02);
						fontSharedMaterial = val2;
					}
