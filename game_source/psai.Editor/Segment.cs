...
{
	private static int DEFAULT_SNIPPET_TYPES = 3;

	private static float DEFAULT_INTENSITY = 0.5f;

	private static float COMPATIBILITY_PERCENTAGE_SAME_GROUP = 1f;
	...
	public void BuildCompatibleSegmentsSet(PsaiProject project)
	{
		...
		foreach (Segment item in segmentsOfAllThemes)
		{
			...
			{
				float compatibility = ((item.Group != Group) ? COMPATIBILITY_PERCENTAGE_OTHER_GROUP : COMPATIBILITY_PERCENTAGE_SAME_GROUP);
	...
	public psai.net.Segment CreatePsaiDotNetVersion(PsaiProject parentProject)
	{
		...
		for (int i = 0; i < CompatibleSnippetsIds.Count; i++)
		{
			int num = CompatibleSnippetsIds.Keys.ElementAt(i);
			segment.Followers.Add(new Follower(num, CompatibleSnippetsIds[num]));
		}
		return segment;
	}
