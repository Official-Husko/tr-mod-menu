	...
	private IEnumerator AnimateVertexColors()
	{
		...
		while (true)
		{
			...
			for (int j = 0; j < lineCount; j++)
			{
				...
				Quaternion val2 = Quaternion.Euler(0f, 0f, Random.Range(-0.25f, 0.25f));
				for (int k = firstCharacterIndex; k <= lastCharacterIndex; k++)
				{
					if (textInfo.characterInfo[k].isVisible)
					{
						...
						Vector3 val3 = (vertices[vertexIndex] + vertices[vertexIndex + 2]) / 2f;
						copyOfVertices[materialReferenceIndex][vertexIndex] = vertices[vertexIndex] - val3;
						copyOfVertices[materialReferenceIndex][vertexIndex + 1] = vertices[vertexIndex + 1] - val3;
						copyOfVertices[materialReferenceIndex][vertexIndex + 2] = vertices[vertexIndex + 2] - val3;
						copyOfVertices[materialReferenceIndex][vertexIndex + 3] = vertices[vertexIndex + 3] - val3;
						float num2 = Random.Range(0.95f, 1.05f);
						Matrix4x4 val4 = Matrix4x4.TRS(Vector3.one, Quaternion.identity, Vector3.one * num2);
						copyOfVertices[materialReferenceIndex][vertexIndex] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 1] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 1]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 2] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 2]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 3] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 3]);
						...
						reference8 -= val;
						val4 = Matrix4x4.TRS(Vector3.one, val2, Vector3.one);
						copyOfVertices[materialReferenceIndex][vertexIndex] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 1] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 1]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 2] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 2]);
						copyOfVertices[materialReferenceIndex][vertexIndex + 3] = ((Matrix4x4)(ref val4)).MultiplyPoint3x4(copyOfVertices[materialReferenceIndex][vertexIndex + 3]);
